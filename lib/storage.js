...
export async function checkCache(videoId) {
  ...
  if (storageMode === 'gcs' && bucket) {
    ...
    const multiplexFile = bucket.file(`processedSongs/${videoId}/multiplex.m4a`);
    const [exists] = await file.exists();
    const [multiplexExists] = await multiplexFile.exists();
    
    if (exists && multiplexExists) {
      try {
...
export async function getFileStream(videoId, stem) {
  const extension = stem === 'multiplex' ? 'm4a' : 'wav';
  const localStemPath = stem === 'multiplex' 
    ? path.join(process.cwd(), 'uploads', videoId, `multiplex.m4a`)
    : path.join(process.cwd(), 'uploads', videoId, 'htdemucs', 'input', `${stem}.wav`);
  const gcsPath = `processedSongs/${videoId}/${stem}.${extension}`;
...
/**
 * Generates a direct Google Cloud Storage CDN URL.
 * By using a public bucket, we bypass Next.js 4MB buffering limits
 * AND avoid the IAM Credentials API metadata signing errors.
 */
export async function getPublicUrl(videoId, stem) {
  const extension = stem === 'multiplex' ? 'm4a' : 'wav';
  const gcsPath = `processedSongs/${videoId}/${stem}.${extension}`;
...
/**
 * Fetches the catalog of all previously processed songs.
 * Reads metadata.json files from processedSongs/ prefix in GCS.
 */
export async function getCachedCatalog() {
  if (storageMode === 'gcs' && bucket) {
    try {
      ...
      // Fetch metadata in parallel batches (e.g., up to 20 at a time) to prevent timeouts
      // Since it's a catalog, we just grab them all for now.
      const fetchPromises = metadataFiles.map(async (file) => {
        try {
          ...
          // parts[0] is 'processedSongs', parts[1] is the videoId
          const videoId = parts[1];
          if (videoId && videoId !== 'metadata.json') {
            data.videoId = videoId;
            data.albumArt = `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;
          }
      ...
      console.error('Failed to list GCS catalog:', e);
      return [];
    }
  } else {
    // Local mode fallback
    try {
      ...
      for (const dir of dirs) {
        ...
        if (fs.existsSync(metadataPath)) {
          try {
            ...
            data.videoId = dir;
            data.albumArt = `https://i.ytimg.com/vi/${dir}/hqdefault.jpg`;
            catalog.push(data);
          } catch(e) {}
        }
      }
      ...
      console.error('Failed to list local catalog:', e);
      return [];
    }
  }
}
