...
export async function runWhisper(jobId, lyricsData, onProgress = null) {
    ...
    // Spawn Python script using the virtual environment or env variable
    const defaultPythonPath = path.join(os.homedir(), 'Desktop', 'GenAIProjects', 'MusicPractice', 'backend', '.venv', 'bin', 'python3');
    ...
    // Ensure lyricsData is an array of objects {text: "..."} for python
    let formattedLyrics = [];
    if (lyricsData) {
        if (Array.isArray(lyricsData)) {
            ...
            const rawText = lyricsData.syncedLyrics || lyricsData.plainLyrics || '';
            formattedLyrics = rawText.split('\n').map(l => ({ text: l.replace(/\[.*?\]/g, '').trim() })).filter(l => l.text);
        }
    }
